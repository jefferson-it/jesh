...
- [x] **Builtins de Controle e Execução:**
  ...
  - [x] `exec` (substituir o processo do shell pelo comando executado, sem criar subprocesso). *(src/builtin/mod.rs:518-534, src/executor/mod.rs:117-150)*
...
### 2. Parser
- [x] **Substituição de Processos (Process Substitution):** Sintaxe `<(comando)` e `>(comando)` para passar saídas/entradas de sub-processos como se fossem arquivos. *(src/parser/lexer.rs:145-154,198-207; src/executor/pipeline.rs:109-165)*
...
- [x] **ANSI-C Quoting:** Sintaxe `$'string'` para interpretar sequências de escape diretamente (ex: `$'linha1\nlinha2'`). *(src/parser/lexer.rs:535-550,868-980)*
- [x] **Continuação de Linha:** Interpretação da barra invertida (`\`) ao final de uma linha física para continuar o mesmo comando na linha seguinte. *(src/utils/mod.rs:342-373; src/main.rs:508-535)*
- [x] **Expansão de Histórico (Bash-style):** Suporte nativo para `!!`, `!$`, `!*`, `!-n` e `!prefixo` no parser. *(src/main.rs:28-168)*
...
- [x] **Status do Pipeline (PIPESTATUS / pipefail):** Um array contendo os códigos de retorno de todos os comandos do pipeline (ex: em `a | b`, capturar se `a` falhou, mesmo se `b` retornar 0). *(src/shell/mod.rs:119-120,211-212,547-552; src/executor/mod.rs:254-261)*
...
- [x] **Operadores de Teste Avançados:** Suporte integrado a condicionais estruturadas `[[ ... ]]` e ao builtin de teste `[ ... ]`. *(src/builtin/mod.rs:500-509,1712-1742)*
...
- [x] **Variáveis Especiais do Shell:** `$IFS`, `$PPID`, `$UID`, `$GROUPS`, `$PWD`, `$OLDPWD`, `$PIPESTATUS`, `$@`, `$*`, `$#`, `$1`..`$9`. *(src/shell/mod.rs:521-588)*
- [x] **Tipagem e Atributos de Variáveis (`declare` / `typeset` / `local` / `readonly` / `getopts`):**
  ...
  - [x] Builtin `local` para escopo de função. *(src/builtin/mod.rs:1207-1287; src/shell/mod.rs:850-906)*
...
- [x] **Expansão e Manipulação Avançada de Parâmetros:**
  ...
  - [x] `${VAR#pattern}`, `${VAR##pattern}`, `${VAR%pattern}`, `${VAR%%pattern}` (remover prefixos/sufixos curtos ou longos) — *(src/shell/mod.rs:680-724)*
  - [x] `${VAR/pattern/replacement}` e `${VAR//pattern/replacement}` (substituição simples e global) — *(src/shell/mod.rs:741-759)*
  - [x] `${!VAR}` (indireção de variáveis) — *(src/shell/mod.rs:659-663)*

### 6. Globbing (Expansão de Caminhos)
- [x] **Extended Globbing (extglob):** Suporte a padrões complexos como `@(...)`, `*(...)`, `+(...)`, `?(...)` e `!(...)`. *(src/shell/mod.rs:1130-1320; src/parser/lexer.rs:430-531)*
- [x] **Flags de Ajuste de Globbing:**
  - [x] `nullglob` (não dar erro ou manter a string caso nenhum arquivo dê match). *(src/shell/mod.rs:122,1116-1117; src/builtin/mod.rs:278-292)*
  - [x] `failglob` (gerar erro se nenhum match for encontrado). *(src/shell/mod.rs:123,1031-1034; src/builtin/mod.rs:278-292)*
  - [x] `dotglob` (fazer com que o `*` capture arquivos ocultos iniciados por ponto). *(src/shell/mod.rs:124,1093-1095; src/builtin/mod.rs:278-292)*
  - [x] `nocaseglob` (globbing insensível a maiúsculas e minúsculas). *(src/shell/mod.rs:125,1077-1081; src/builtin/mod.rs:278-292)*
...
### 9. Jobs
- [x] **Notificação Assíncrona de Mudança de Estado:** Notificar o usuário imediatamente quando um processo em background finaliza ou para, sem aguardar o próximo comando. *(src/shell/mod.rs:226-249)*
- [x] **Isolamento de Process Groups (PGID):** Proteção do shell contra sinais como `Ctrl+C` (SIGINT) emitidos para processos foreground. *(src/main.rs:271-274; src/executor/pipeline.rs:281-311)*

### 10. Scripts e Execução
- [x] **`set -o pipefail`.** *(src/shell/mod.rs:120; src/builtin/mod.rs:274-277; src/executor/mod.rs:255-259)*
- [x] **Escopo de Função e a palavra-chave `local`:** Variáveis declaradas dentro de funções não vazam para o escopo global. *(src/builtin/mod.rs:1207-1287; src/shell/mod.rs:850-906)*
...
- [ ] **Configuração de Atalhos (Custom Keybindings):** Permitir mapear teclas personalizadas no arquivo `config.toml`. *(Atualmente só bindings hardcoded em main.rs:416-449)*
- [ ] **Yank-ring (Histórico de Recortes):** Suporte a múltiplos níveis de colar no estilo Emacs (`Alt+Y` após `Ctrl+Y`). *(Sem binding explícito para Alt+Y)*
- [ ] **Redesenho de Tela em Redimensionamento:** Manipulação inteligente de `SIGWINCH` para re-renderizar a linha de comando sem quebrar a tela quando o terminal for redimensionado. *(SIGWINCH não é capturado)*
- [ ] **Medição de Largura de Caracteres Unicode (East Asian Width):** Correção do cálculo do cursor para emojis e caracteres asiáticos (double-width). *(Todo cálculo usa `len()` ingênuo—CJK/emoji quebram o cursor)*
- [ ] **Mudança Visual do Cursor por Modo:** Cursor como bloco sólido `█` no modo comando do Vi, e como barra vertical `|` no modo inserção. *(Nenhuma lógica de cursor_style implementada)*

### 2. Globbing
- [ ] **Qualificadores de Globbing (Zsh-style):** Filtrar matches por tipo, ex: apenas diretórios (`*(/)`) ou apenas links simbólicos.

### 3. Jobs
- [ ] **Comando `disown`:** Desvincular um processo em background do shell pai para mantê-lo rodando após a saída do shell. *(Registrado em is_builtin() mas sem handler—cai em "command not found")*

### 4. Scripts e Execução
- [ ] **`set -e`** (parar script no primeiro erro). *(Não implementado—sem campo errexit no ShellState)*
- [ ] **`set -u`** (parar se houver variável não declarada). *(Não implementado—get_var retorna string vazia silenciosamente)*
- [ ] **`set -x`** (imprimir cada linha executada para depuração). *(Nenhum traço de xtrace no código)*

### 5. Prompt
- [ ] **Prompt Direito (RPROMPT):** Informações adicionais exibidas na borda direita do terminal, que desaparecem se a linha de comando crescer muito. *(Função render_rprompt() existe mas nunca é chamada—código morto)*
...
### 6. Temas (Theming)
- [ ] **Sistema de Temas:** Carregamento de temas visuais para personalizar cores, estilos e aparência do shell.
  - [ ] **Variável `$THEME`:** Definição no `.jeshrc` via `THEME="nome-do-tema"` para ativar um tema.
  - [ ] **Diretório Padrão de Temas:** Busca automática em `~/.local/jesh/themes/<nome>.sh` (ou `$XDG_DATA_HOME/jesh/themes/<nome>.sh`).
  - [ ] **Temas Customizados (`source`):** Carregamento de qualquer script shell como tema via `source /caminho/para/tema.sh`.
  - [ ] **Controle Total pelo Tema:** O script de tema pode manipular:
  ...
  - [ ] **Tema Padrão (Fallback):** Se `$THEME` não for definida, não for encontrada, ou carregar com erro, usar tema padrão embutido no shell.
  - [ ] **Exemplos de Temas:** Fornecer temas de exemplo no repositório (ex: `jesh-default`, `jesh-dark`, `jesh-light`, `jesh-dracula`).
...
- [ ] **Smart Paste (Colar Inteligente):** Escapar caracteres perigosos (como `?` ou `&`) ao colar URLs ou textos contendo delimitadores do shell. *(Detecção funciona em utils/mod.rs:606-608, mas o escape é um no-op: text.to_string())*
- [ ] **Substituições Automáticas por CLI Modernas:** Configurações automáticas para usar ferramentas modernas de CLI quando disponíveis (ex: `eza`/`exa` em vez de `ls`, `bat` em vez de `cat`, `zoxide` em vez de `cd`). *(Parcial: eza/bat/rg/fd/htop implementados em shell/mod.rs:30-51; `zoxide` ausente)*
